...
* `root_Pconstructible_of_two_conjugate_pairs_monic` handles `s ≥ 2`: from the cone point
  and the definite two-planes one builds a totally isotropic plane through the cone point
  (`exists_cone_point_Pconstructible`, `exists_orth_neg_Pconstructible`,
  `exists_isotropic_orth_Pconstructible`), kills the top three resolvent coefficients, and
  finishes with the power-law engine;
* `root_Pconstructible_of_one_conjugate_pair` handles `s = 1`: a P-constructible point
  `P₀` on the light cone (the same `exists_cone_point_Pconstructible`), the conjugation sign
...
`qform A x = x ⬝ᵥ (A *ᵥ x)` and `bilin A x y = x ⬝ᵥ (A *ᵥ y)` are the quadratic
and bilinear forms of a matrix `A`, used for the trace form on `{p1 = 0}`. -/
...
/-! ### Nonconstant from vanishing trace

Used by the `s ≥ 2` and `s = 1` tails: a nonzero polynomial with vanishing trace at
`companion7 q` cannot be constant. -/
